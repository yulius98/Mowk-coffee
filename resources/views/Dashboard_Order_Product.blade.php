                ...
                <table class="min-w-full divide-y divide-gray-900">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @foreach ($data_shipping as $dtshipping)
                            <tr>
                                <td class="border border-gray-900 px-4 py-2 text-[rgb(4,4,4)]">{{ $dtshipping->order_id }}</td>
                                <td class="border border-gray-900 px-4 py-2 text-[rgb(4,4,4)]">{{ $dtshipping->nama_pembeli }}</td>
                                <td class="border border-gray-900 px-4 py-2 text-[rgb(4,4,4)]">{{ $dtshipping->nama_product }}</td>
                                <td class="border border-gray-900 px-4 py-2 text-[rgb(4,4,4)]">{{ $dtshipping->jumlah_product }}</td>
                                <td class="border border-gray-900 px-4 py-2 text-[rgb(4,4,4)]">Rp {{ number_format((float)$dtshipping->total_price, 0, ',', '.') }}</td>
                                <td class="border border-gray-900 px-4 py-2 text-[rgb(4,4,4)]">{{ $dtshipping->alamat_pengiriman }}</td>
                                <td class="border border-gray-900 px-4 py-2 text-[rgb(4,4,4)]">{{ $dtshipping->no_HP }}</td>
                                <td class="border border-gray-900 px-4 py-2 text-gray-500">
                                    <div class="d-flex justify-content-between mb-3">
                                        <button class="btn btn-danger gap-x-1.5 rounded-md bg-[rgba(178,45,45,0.87)] px-3 py-2 text-sm font-semibold text-white shadow-sm ring-1 ring-inset ring-gray-800 hover:bg-gray-700"
                                            onclick="window.location.href='/shipping_product/{{ $dtshipping->id }}/{{ $title }}'">
                                            Shipping Product
                                        </button>
                                    </div>
                                </td>
    ...
    <div class="container mx-auto mt-4 px-4 py-8">
        ...
        <div class="card-body bg-transparent">
            <table class="min-w-full divide-y divide-gray-900">
                ...
                <tbody class="divide-y divide-gray-200">
                    <!-- Loop through all_status_order data -->
