<x-layout-cruid-seller>
    <x-slot:title>{{ $title }}</x-slot:title>
        ...
        <div class="mt-3">
            <h3 class=" font-bold text-lg leading-6 text-black mt-20">Edit Event</h3>
            <div class="bg-[rgb(236,222,210)] rounded-xl shadow-lg overflow-hidden transform p-6 max-w-md">
                ...
                @endif
                <form id="EditEventForm" class="space-y-6" action="/edit_event/{{ $title }}" method="post" enctype="multipart/form-data">
                    @csrf
                    
                    <div>
                        <input type="hidden" id="nama_seller" name="nama_seller" class="mt-1 block" value="{{ $title }}">
                        <input type="hidden" id="id" name="id" class="mt-1 block" value="{{ $data_event->id }}">
                    </div>

                    <div>
                        <label class="block text-sm font-bold text-black">Event Date</label>
                        <input type="date" id="date_event" name="date_event" value="{{ $data_event->date_event }}" required 
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 p-2">
                    </div>

                    <div>
                        <label class="block text-sm font-bold text-black">Event Name</label>
                        <input type="text" id="name_event" name="name_event" value="{{ $data_event->name_event }}" required 
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 p-2">
                    </div>
                    ...
                    <div>
                        ...
                        <textarea id="description_event" name="description_event" required 
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 p-2">{{ $data_event->description_event }}</textarea>
                    </div>

                    <div>
                        <label class="block text-sm font-bold text-black">Event Venues</label>
                        <input type="text" id="location_event" name="location_event" value="{{ $data_event->location_event }}" required 
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 p-2">
                    </div>
                    ...
                    <div>
                        ...
                        <div class="flex items-center space-x-4">
                            <label class="inline-flex items-center">
                                <input type="radio" id="ticket_free" name="tiket" value="{{ $data_event->tiket }}" required
                                    class="text-blue-500 focus:ring-blue-500">
                                <span class="ml-2 text-sm text-black">Free</span>
                            </label>
                            <label class="inline-flex items-center">
                                <input type="radio" id="ticket_paid" name="tiket" value="{{ $data_event->tiket }}" required
                                    class="text-blue-500 focus:ring-blue-500">
                    ...
                    <div>
                        ...
                        <input type="number" id="harga_tiket" name="harga_tiket" value="{{ $data_event->harga_tiket }}" 
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 p-2">
                    </div>
